...
def _create_response_boundary_classes_for_filepath(filepath_src: str, filepath_dest: str) -> None:
    # Open original data
    raster_src = gdal.Open(filepath_src)
    band_src = raster_src.GetRasterBand(1)
    original = band_src.ReadAsArray()
    # Add boundaries
    with_boundaries = _add_class_boundaries(original)
    # Write output file
...
def _add_class_boundaries(original: np.array) -> np.array:
    # Calculate edges
    ...
    for class_ in classes:
        ...
        class_edges[class_] = _get_edges(class_image)
    del class_image
