...
def create_landsat_quad_vrts() -> None:
    ...
    if not os.path.exists(filepath_landsat_global):
        ...
        filepaths_landsats = [f for f in os.listdir(paths.DIR_DATA_LANDSAT_ORIG) if re.search('\d{10}-\d{10}.tif$', f)]
        filepath_inputs = 'input_list.txt'
        with open(filepath_inputs, 'w') as file_:
            file_.writelines('\n'.join(filepaths_landsats))
        command = 'gdalbuildvrt -a_srs EPSG:{srs_out} -input_file_list {filepath_inputs} {filepath_out}'.format(
            srs_out=EPSG_DEST, filepath_inputs=filepath_inputs, filepath_out=filepath_landsat_global)
        command_line.run_command_line(command)
        os.remove(filepath_inputs)
        os.chdir(cw_dir)

    # Create quad vrts
    dir_out = os.path.join(paths.SUBDIR_DATA_TRAIN_DOWNSAMPLE.format(50))
    for filepath_quad in tqdm(filepaths_quads):
        filepath_out = os.path.join(dir_out, os.path.splitext(os.path.basename(filepath_quad))[0] + '_50.tif')
        # Get vrt parameters
        ...
        lly = min([y0, y1])
        ury = max([y0, y1])
        command = 'gdalbuildvrt -tr {xres} {yres} -te {llx} {lly} {urx} {ury} -vrtnodata -9999 -a_srs {srs_out} ' + \
                  '-r average {filepath_out} {filepath_src}'
        command = command.format(
            xres=xres, yres=yres, llx=llx, lly=lly, urx=urx, ury=ury, srs_out=EPSG_DEST, filepath_out=filepath_out,
